...
- **No held-out validation split.** The model trains on all annotated samples rather than reserving a portion for local evaluation, so there is no internal validation metric tracked during training. Per-sample ground truth does exist (the bundled demo samples use it to show predicted-vs-actual boxes), but it was never carved into a dedicated test split. The headline IoU of 0.626 comes from the competition leaderboard, which scored a hidden test set; validation during the hackathon was therefore indirect, via leaderboard submissions.
